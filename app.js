...
function ProductTable ({products, filterText, inStockOnly}) {
    ...
    const filterTextLowercase = filterText.toLowerCase(); // convertit en minuscule
    products.forEach((product) => {
        if (inStockOnly && !product.stocked) {
            return;
        }
        const productNameLowercase = product.name.toLowerCase(); // convertit en minuscule
        if (productNameLowercase.indexOf(filterTextLowercase) === -1) {
            return;
        }
